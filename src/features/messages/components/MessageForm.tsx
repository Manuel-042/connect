...
const MessageForm = () => {
    ...
    return (
        <div className="relative flex-grow">
            ...
            <div className="py-2 w-full" onClick={e => e.stopPropagation()}>
                <form className="flex items-center justify-between w-[95%] mx-auto dark:bg-[#202327] rounded-md">
                    ...
                    <div className={`w-[80%] ${isAttachment && 'flex flex-col items-start gap-2 py-2 ps-2'}`}>
                        {isAttachment &&
                            (
                                attachmentType === "image" ? file && <MediaPreview media={file} onRemove={onRemove} /> : gifPreview && <MediaPreview media={gifPreview} onRemove={onRemove} />
                            )
                        }
                        <textarea
                            name="message"
